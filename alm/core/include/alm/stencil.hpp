...
class Stencil {
 public:
  ...
  [[nodiscard]] constexpr const Frame &future() const {
    return frames_[order_[static_cast<std::size_t>(SliceRole::kFuture)]];
  }
  [[nodiscard]] constexpr const Frame &now() const { return frames_[order_[static_cast<std::size_t>(SliceRole::kNow)]]; }
  [[nodiscard]] constexpr const Frame &recent() const {
    return frames_[order_[static_cast<std::size_t>(SliceRole::kRecent)]];
  }
  [[nodiscard]] constexpr const Frame &stable() const {
    return frames_[order_[static_cast<std::size_t>(SliceRole::kStable)]];
  }
